...
public class MageLoginTask extends AbstractAccountTask {

	private static final String LOG_NAME = MageLoginTask.class.getName();
	private DateFormat iso8601Format = ISO8601DateFormatFactory.ISO8601();
	private UserDeserializer userDeserializer;

	public MageLoginTask(AccountDelegate delegate, Context context) {
	...
	private AccountStatus login(String... params) {
		...
		Boolean needToRegisterDevice = Boolean.valueOf(params[2]);
			...
			try {
				...
				if (oldUsername != null && oldPasswordHash != null && !oldPasswordHash.trim().isEmpty()) {
					if (oldUsername.equals(username) && PasswordUtility.equal(password, oldPasswordHash)) {
						...
						sharedPreferences.edit().putString(mApplicationContext.getString(R.string.tokenExpirationDateKey), iso8601Format.format(tokenExpiration)).apply();
		...
		try {
			...
			Response<JsonObject> response = userResource.signin("local", parameters);
