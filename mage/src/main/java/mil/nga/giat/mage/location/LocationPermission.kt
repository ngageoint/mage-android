...
class LocationPermission : ActivityResultContract<Void?, LocationContractResult>() {
   ...
   override fun parseResult(resultCode: Int, intent: Intent?): LocationContractResult {
      if (resultCode != Activity.RESULT_OK) {
         return LocationContractResult(preciseGranted = false, coarseGranted = false)
      }
      ...
      if (grantResults == null || grantResults.isEmpty() || permissions.isNullOrEmpty()) {
         return LocationContractResult(preciseGranted = false, coarseGranted = false)
      }
      ...
      return LocationContractResult(preciseGranted = preciseGranted, coarseGranted = coarseGranted)
   }
}