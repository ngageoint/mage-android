...
class ObservationDeserializer(val context: Context) {
   ...
   @Throws(IOException::class)
   private fun readProperties(reader: JsonReader, observation: Observation): Collection<ObservationProperty> {
         ...
         when(reader.peek()) {
            JsonToken.BEGIN_OBJECT -> {
               geometryDeserializer.read(reader)?.let { geometry ->
                  val geometryBytes = GeometryUtility.toGeometryBytes(geometry)
            ...
            JsonToken.BEGIN_ARRAY -> {
               ...
               if (stringArrayList.isNotEmpty()) {
                  property = ObservationProperty(key, stringArrayList)
               } else if (attachments.isNotEmpty()) {
                  property = ObservationProperty(key, attachments)
               }
            ...
            JsonToken.NUMBER -> {
               reader.nextNumberOrNull()?.let {
                  property = ObservationProperty(key, it)
               }
            }
            JsonToken.BOOLEAN -> {
   ...
   @Throws(IOException::class)
   private fun readFavoriteUsers(reader: JsonReader): Collection<ObservationFavorite> {
      ...
      while(reader.hasNext()) {
         if (reader.peek() == JsonToken.STRING) {
            ...
            val favorite = ObservationFavorite(userId, true)
